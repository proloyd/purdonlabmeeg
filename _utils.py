...
def _viz_score_correlations(orig_scores, orig_evs, pa_evs, fig):
    ...
    for i in range(orig_scores.shape[0]):
    	for j in range(i+1):
    		...
    		ax.stem(lags, score_corr / np.sqrt(np.abs(orig_evs[i] * orig_evs[j])), markerfmt=',')
    		ax.set_xlim([-xlim, xlim])
    		if i == j: ax.set_title(f'{orig_evs[i]:.2}({pa_evs[i]:.2})')
    		if j == 0: ax.set_ylabel(f'y{i}')
    		if i == orig_scores.shape[0]-1: ax.set_xlabel(f'y{j}')
    		ax.set_ylim([-1, 1])
    plt.tight_layout()
    fig.canvas.draw()
...
def inspect_order(x, max_order=20):
	# 1. Set `1 = 0`.
	l = 0
	rs = []
	r_news = []
	...
	while True:
		...
		last_principle_ev_ = input("Enter last principle eigen-value number (in pythonic numbering): ")
		...
		this_r_new = this_r - sum((l - np.arange(l) + 1) * np.asanyarray(r_news))
		r_news.append(this_r_new)
		print(f'{l} \t {X.shape[0]} \t {last_principle_ev + 1} \t {this_r} \t {this_r_new}')
