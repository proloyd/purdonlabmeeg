...
class TimeSeriesData:
    def __init__(self, num_datapoints, test_size=0.2, max_t=20, num_prev=1,
        ...
        :param test_size: in (0,1), data to be used in test set as a fraction
    ...
    def train_test_split(self):
        ...
        self.bayes_preds = [self.bayes_train_preds, self.bayes_test_preds] =\
             self.bayes_preds[:-test_size], self.bayes_preds[-test_size:]
...
class ARData(TimeSeriesData):
    ...
    def generate_data(self):
        ...
        # + 3*self.num_prev because we want to cut first (3*self.num_prev)
        # datapoints later so dist is more stationary (else initial num_prev
        # datapoints will stand out as diff dist)
        for i in range(self.num_datapoints+3*self.num_prev):
            # Generate y value if there was no noise
            # (equivalent to Bayes predictions: predictions from oracle
            # that knows true parameters (coefficients))
            self.bayes_preds[i + self.num_prev] = \
                  np.dot(self.y[i:self.num_prev+i][::-1], self.coeffs)
            # Add noise
...
# A set of coefficients that are stable (to produce replicable plots,
# experiments)
fixed_ar_coefficients = {2: [0.46152873, -0.29890739],
                         5: [0.02519834, -0.24396899,  0.2785921, 0.14682383,
                             0.39390468],
                         10: [-0.10958935, -0.34564819,  0.3682048,  0.3134046,
                              -0.21553732,  0.34613629, 0.41916508,  0.0165352,
                              0.14163503, -0.38844378],
                         20: [0.1937815, 0.01201026, 0.00464018, -0.21887467,
                              -0.20113385, -0.02322278, 0.34285319, -0.21069086,
                              0.06604683, -0.22377364,  0.11714593, -0.07122126,
                              -0.16346554,  0.03174824,  0.308584, 0.06881604,
                              0.24840789, -0.32735569, 0.21939492, 0.3996207]}

"""
Example of using fixed coefficients (consistency across tests
                                     of different models)
